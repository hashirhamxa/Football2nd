...
import livefootball.footballstreamning.fifaworldcup.adapters.TournamentEventAdapter
...
//        )
        ...
        findViewById<ImageButton>(R.id.btn_back).setOnClickListener { 
            AdsHelper.getInstance(this@EventsListActivity).showAd_Mob_X_Inter_With_Time(this@EventsListActivity)
            finish() 
        }
                ...
                launch {
                    if (isHighlightsMode) {
                        viewModel.highlightEvents.collectLatest { events ->
                            rvEvents.adapter = TournamentEventAdapter(events, tournamentName, true, tournamentThumbUrl)
                        }
                    } else {
                        viewModel.events.collectLatest { events ->
                            rvEvents.adapter =
                                TournamentEventAdapter(events, tournamentName, false, tournamentThumbUrl)
                        }
                    }
                }
        ...
        lifecycleScope.launch {
            ...
            ads.find { it.adPlacement.equals("Banner", ignoreCase = true) }?.let { ad ->
                if (ad.isActive == true && !ad.adUnitId.isNullOrEmpty()) {
                    ...
                    AdsHelper.getInstance(this@EventsListActivity).loadAdaptiveADMOB_X_Banner(this@EventsListActivity, adContainer, ad.adUnitId)
                }
            }
            ...
            ads.find { it.adPlacement.equals("Interstitial", ignoreCase = true) }?.let { ad ->
                if (ad.isActive == true && !ad.adUnitId.isNullOrEmpty()) {
                    AdsHelper.getInstance(this@EventsListActivity).preloadAdADMOB_X_Inter(this@EventsListActivity, ad.adUnitId)
                }
            }
            ...
            ads.find { it.adPlacement.equals("Rewarded", ignoreCase = true) }?.let { ad ->
                if (ad.isActive == true && !ad.adUnitId.isNullOrEmpty()) {
                    AdsHelper.getInstance(this@EventsListActivity).preloadRewardedAd(this@EventsListActivity, ad.adUnitId)
                }
            }
        }
    }
