...
class BrightnessControlHandler {

    private final Activity activity;

    public int currentBrightnessLevel = -1;

    public BrightnessControlHandler(Activity activity) {
    ...
    public void setPlayerScreenBrightness(final float brightness) {
        WindowManager.LayoutParams lp = activity.getWindow().getAttributes();
        lp.screenBrightness = brightness;
        activity.getWindow().setAttributes(lp);
    }

    public void changeViewBrightness(final CustomPlayerView playerView, final boolean increase, final boolean canSetAuto) {
        ...
        if (currentBrightnessLevel == -1 && canSetAuto)
            setPlayerScreenBrightness(WindowManager.LayoutParams.BRIGHTNESS_OVERRIDE_NONE);
        else if (currentBrightnessLevel != -1)
            setPlayerScreenBrightness(levelBrightness(currentBrightnessLevel));
    ...
    float levelBrightness(final int level) {
