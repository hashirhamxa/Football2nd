...
class Utility {
    ...
    public static void adjustVolume(final Context context, final AudioManager audioManager,
                                    ...
                                    boolean canBoost, boolean clear) {
        ...
        try {
            if (volume != volumeMax || (PlayerActivity.boostLevel == 0 && !raise)) {
                ...
                try {
                    ...
                    Log.e("Utility", "Volume adjust blocked: " + se.getMessage());
                ...
                if (playerView.volumeUpsInRow > 4 && !isVolumeMin(audioManager)) {
                    try {
                        ...
                        Log.e("Utility", "Fine volume adjust blocked: " + se.getMessage());
                    }
                } else {
            ...
            Log.e("Utility", "Unexpected error adjusting volume: " + e.getMessage());
        }
    ...
    public static void setButtonEnabled(final Context context, final ImageButton button, final boolean enabled) {
        ...
        button.setAlpha(enabled ?
                (float) context.getResources().getInteger(androidx.media3.ui.R.integer.exo_media_button_opacity_percentage_enabled) / 100 :
                (float) context.getResources().getInteger(androidx.media3.ui.R.integer.exo_media_button_opacity_percentage_disabled) / 100
        );
    }
...
//                                    "Current display refreshRate: " + modeBest.getRefreshRate(), Toast.LENGTH_LONG).show();
    ...
    private static void playIfCan(final PlayerActivity activity, boolean play) {
        if (play) {
            ...
            if (activity.customPlayerView != null)
                activity.customPlayerView.hideController();
        }
    }
    ...
    public static boolean alternativeChooser(PlayerActivity activity, Uri initialUri, boolean video) {
        ...
        ChooserDialog chooserDialog = new ChooserDialog(activity, com.obsez.android.lib.filechooser.R.style.FileChooserStyle_Dark)
                ...
                .withChosenListener(new ChooserDialog.Result() {
                    ...
                    public void onChoosePath(String path, File pathFile) {
                        ...
                        if (video) {
                            activity.spHandler.setPersistent(true);
                            activity.spHandler.updateMedia(activity, uri, null);
                            activity.searchVideoSubtitles();
                        } else {
                            // Convert subtitles to UTF-8 if necessary
                            SubtitleUtility.clearCache(activity);
                            uri = SubtitleUtility.convertToUTF(activity, uri);

                            activity.spHandler.updateSubtitle(uri);
                        }
