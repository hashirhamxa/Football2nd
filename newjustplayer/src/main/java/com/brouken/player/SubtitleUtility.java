...
class SubtitleUtility {
    ...
    */
    ...
    public static boolean isSubtitle(Uri uri, String mimeType) {
        if (mimeType != null) {
            for (String mime : Utility.supportedMimeTypesSubtitle) {
        ...
        if (uri != null) {
            if (Utility.isSupportedNetworkUri(uri)) {
                String path = uri.getPath();
                if (path != null) {
                    path = path.toLowerCase();
                    for (String extension : Utility.supportedExtensionsSubtitle) {
    ...
    public static Uri convertToUTF(PlayerActivity activity, Uri subtitleUri) {
        try {
            ...
            if (scheme != null && scheme.toLowerCase().startsWith("http")) {
                ...
                SubtitleFetcherHandler subtitleFetcherHandler = new SubtitleFetcherHandler(activity, urls);
                subtitleFetcherHandler.start();
                return null;
            } else {
    ...
    public static MediaItem.SubtitleConfiguration buildSubtitle(Context context, Uri uri, String subtitleName, boolean selected) {
        final String subtitleMime = SubtitleUtility.getSubtitleMime(uri);
        final String subtitleLanguage = SubtitleUtility.getSubtitleLanguage(uri);
        if (subtitleLanguage == null && subtitleName == null)
            subtitleName = Utility.getFileName(context, uri);
